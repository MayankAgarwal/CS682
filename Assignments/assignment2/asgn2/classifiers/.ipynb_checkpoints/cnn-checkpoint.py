...
class ThreeLayerConvNet(object):
  ...
  def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
    ...
    # The way conv layer is set, it won't change the spatial dimension of the input
    # Only the max-pool layer would reduce the dimensions by half
    hidden_layer_dim = (num_filters*H*W)/4
    
    self.params['W1'] = np.random.normal(0, scale=weight_scale, size=(num_filters, C, filter_size, filter_size))
    self.params['b1'] = np.zeros(num_filters)
    
    self.params['W2'] = np.random.normal(0, scale=weight_scale, size=(hidden_layer_dim, hidden_dim))
